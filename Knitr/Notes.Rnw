...
There were \Sexpr{sum(da$Clear == 1)} students taken  onto the course through clearing.  It is evident from the box-plot in Figure \ref{boxplot1} and from Table \ref{tabref:out2} that these were relatively strong students.   

<<boxplot1, echo=FALSE, fig.height=4, fig.width=3, fig.cap="Comparison of total and clearing grades", fig.pos="h">>=
...
# da$Outcomeprop <- as.numeric(levels(da$Outcome))[da$Outcome]
nicetable2 <- xtable(table(daclear$Outcome, dnn = "Number of Students"), caption = "Outcome of Level 4 BSc. Finance and Investsment Students joined throgh clearing", label = "tabref:out2")
nicetable2
@

\subsection*{Entry grades and performance}
<<boxplot2, echo=FALSE, fig.height=4, fig.width=3, fig.cap="Entry grades and student performance", fig.pos="h">>=
...
<<boxplot3, echo=FALSE, fig.height=4, fig.width=3, fig.lp="figref:box2">>=
daover <- subset(da, da$Fee.status == "O")
par(las = 2)
