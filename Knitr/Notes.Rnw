...
@
The total number of students was \Sexpr{length(da$Mark)}.  The outcomes at the summer board are in Table \ref{tabref:out}.  
...
nicetable
@
The figures show that \Sexpr{table(da$Outcome)[1]/length(da$Outcome)*100} percent of the students failed and were asked to withdraw;  \Sexpr{table(da$Outcome)[2]/length(da$Outcome)*100} percent passed at the first attempt; \Sexpr{table(da$Outcome)[3]/length(da$Outcome)*100} were referred and \Sexpr{table(da$Outcome)[4]/length(da$Outcome)*100} withdrew before taking exams.  

There are four issues to consider in detail: the first is the proportion of student who do not complete the first year; the second is the performance of students taken onto the course through clearing; the third is the relationship between student entry grades and the level four performance; the final issue is that of the perforfmance of overseas students. 
...
There were \Sexpr{sum(da$Clear == 1)} students taken  onto the course through clearing.  
...
# da$Outcomeprop <- as.numeric(levels(da$Outcome))[da$Outcome]
nicetable2 <- xtable(table(daClear$Outcome, dnn = "Number of Students"), caption = "Outcome of Level 4 BSc. Finance and Investsment Students joined throgh clearing", label = "tabref:out")
nicetable2
@

<<boxplot1, echo=FALSE, fig.height=5, fig.width=4>>=
daclear <- subset(da, da$Clear == 1)
boxplot(da$Mark, daclear$Mark, names = c("Full Group", "Clearing"), main = "Grade Distribution of Student Grades")
@

<<boxplot2, echo=FALSE, fig.height=5, fig.width=4>>=
...
nicetable3 <- xtable(lm(da$Mark ~ da$Alevel -1), label = "Eqref:1", caption = "Level 4 grade for A-level category", digits = 2)
nicetable3
@
\end{document}